...
import { modeloCanvasTranslations } from "@/public/locales/modelo-canvas/modelo-canvas";

export async function generateMetadata({ params: { lang } }: { params: { lang: Language } }): Promise<Metadata> {
    const t = modeloCanvasTranslations[lang];
    return t.metadata;
}

const ModeloCanvas = ({ params: { lang } }: { params: { lang: Language } }) => {
    const t = modeloCanvasTranslations[lang];
        ...
        <div>
            <Breadcrumb className="xl:px-[15%] lg:px-[7%] md:px-[5%] px-[5%] pt-5 bg-blue-600">
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage className="text-white">{t.breadcrumbs.modeloCanvas}</BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            <BlogStructure
