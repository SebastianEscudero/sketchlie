...
import { queEsModeloCanvasTranslation } from "@/public/locales/modelo-canvas/que-es-modelo-canvas";

export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
    const translation = queEsModeloCanvasTranslation[params.lang as keyof typeof queEsModeloCanvasTranslation];
    return {
...
const LandingPage = ({ params }: { params: { lang: string } }) => {
    const t = queEsModeloCanvasTranslation[params.lang as keyof typeof queEsModeloCanvasTranslation];
            ...
            <div className="mt-[3%]">
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <Link href={`/${params.lang}/modelo-canvas/`} title={t.breadcrumbs.modeloCanvas}>{t.breadcrumbs.modeloCanvas}</Link>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-white">{t.breadcrumbs.queEsModeloCanvas}</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </div>
