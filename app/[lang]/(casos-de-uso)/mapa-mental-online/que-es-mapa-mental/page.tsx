...
import { QueEsMapaMentalTranslation } from "@/public/locales/mapa-mental/que-es-mapa-mental";
import { Language } from "@/types/canvas";

export async function generateMetadata({ params }: { params: { lang: Language } }): Promise<Metadata> {
    const translation = QueEsMapaMentalTranslation[params.lang];
    return {
...
const LandingPage = ({ params }: { params: { lang: Language } }) => {
    const t = QueEsMapaMentalTranslation[params.lang];
            ...
            <div className="mt-[3%]">
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <Link href={`/${params.lang}/mapa-conceptual/`} title={t.breadcrumbs.mapaMental}>{t.breadcrumbs.mapaMental}</Link>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-black">{t.breadcrumbs.queEsMapaMental}</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </div>
