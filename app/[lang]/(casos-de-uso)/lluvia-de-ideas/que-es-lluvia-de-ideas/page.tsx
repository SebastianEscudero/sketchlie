...
import { queEslluviaDeIdeasTranslations } from "@/public/locales/lluvia-de-ideas/que-es-lluvia-de-ideas";
import React from "react";

export async function generateMetadata({ params: { lang } }: { params: { lang: Language } }): Promise<Metadata> {
    const t = queEslluviaDeIdeasTranslations[lang];
    return t.metadata;
}
            ...
            <div className="mt-[3%]">
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <Link href={`/${lang}/lluvia-de-ideas/`} title={t.breadcrumbs.lluviaDeIdeas}>{t.breadcrumbs.lluviaDeIdeas}</Link>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-black">{t.breadcrumbs.queEs}</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </div>
                    ...
                    <ul>
                        {t.sections.map((section) => (
                            <li key={section.id} className="mb-4">
                                {React.createElement(Link, { href: `#${section.id}`, className: "text-custom-blue hover:underline mb-10" }, section.title)}
                            </li>
                        ))}
                    </ul>
                </div>
