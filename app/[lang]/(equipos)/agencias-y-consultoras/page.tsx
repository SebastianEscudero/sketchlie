...
import { agenciasConsultorasTranslations } from "@/public/locales/agencias-y-consultoras/agencias-y-consultoras";

export async function generateMetadata({ params: { lang } }: { params: { lang: Language } }): Promise<Metadata> {
    const t = agenciasConsultorasTranslations[lang];
    return t.metadata;
}

const AgenciasYConsultoras = ({ params: { lang } }: { params: { lang: Language } }) => {
    const t = agenciasConsultorasTranslations[lang];
        ...
        <div>
            <Breadcrumb className="xl:px-[15%] lg:px-[7%] md:px-[5%] px-[5%] pt-5 bg-blue-600">
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage className="text-white">{t.breadcrumbs.agenciasConsultoras}</BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            <BlogStructure
...
export default AgenciasYConsultoras;