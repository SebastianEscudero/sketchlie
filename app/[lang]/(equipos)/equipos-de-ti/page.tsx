...
import { equiposTITranslations } from "@/public/locales/equipos-de-ti/equipos-de-ti";

export async function generateMetadata({ params: { lang } }: { params: { lang: Language } }): Promise<Metadata> {
    const t = equiposTITranslations[lang];
    return t.metadata;
}

const EquiposDeTi = ({ params: { lang } }: { params: { lang: Language } }) => {
    const t = equiposTITranslations[lang];
        ...
        <div>
            <Breadcrumb className="xl:px-[15%] lg:px-[7%] md:px-[5%] px-[5%] pt-5 bg-blue-600">
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage className="text-white">{t.breadcrumbs.equiposTI}</BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            <BlogStructure
