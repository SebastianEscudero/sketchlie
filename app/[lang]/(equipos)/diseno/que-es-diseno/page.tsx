...
import { queEsDisenoTranslation } from "@/public/locales/diseno/que-es-diseno";

export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
    const translation = queEsDisenoTranslation[params.lang as keyof typeof queEsDisenoTranslation];
    return {
...
const LandingPage = ({ params }: { params: { lang: string } }) => {
    const t = queEsDisenoTranslation[params.lang as keyof typeof queEsDisenoTranslation];
            ...
            <div className="mt-[3%]">
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <Link href={`/${params.lang}/diseno/`} title={t.breadcrumbs.diseno}>{t.breadcrumbs.diseno}</Link>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbPage className="text-white">{t.breadcrumbs.queEsDiseno}</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </div>
